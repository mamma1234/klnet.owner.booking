...
export default function CustomTable(props) {
  ...
  const { tableHead, tableData, tableHeaderColor,colSpan,tableHeaderColorCode } = props;
  ...
  return (
    <div className={classes.tableResponsive}>
      <Table className={classes.table}>
        {tableHead !== undefined ? (
          <TableHead className={classes[tableHeaderColor + "TableHeader"]} style={{backgroundColor:tableHeaderColorCode}}>
            <TableRow className={classes.tableHeadRow}>
              {tableHead.map((prop, key) => {
                return (
                  <TableCell
                    className={classes1.cellStyle}
					key={key}
                  >
                    {prop}
                  </TableCell>
        ...
        {(rowsPerPage > 0?tableData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) :  tableData).map((prop, key) => {
            return (
              <TableRow key={key} className={classes.tableBodyRow}>
                {prop.map((prop, key) => {
                  return (
                    <TableCell className={classes1.cellStyle} key={key}>
                      {prop}
                    </TableCell>
                  );
                })}

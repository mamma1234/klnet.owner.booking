		...
		<Card className={classes.paper} id="booking">
						...
						<GridContainer>
							...
							<GridItem xs={8} sm={7}>
								<Autocomplete
									...
									value = {{other_bookmark_name:store.sr_bookmark.bookingList && store.sr_bookmark.bookingList.find(v=>v.value === store.sr.other_bookmark_seq)?store.sr_bookmark.bookingList.find(v=>v.value === store.sr.other_bookmark_seq).other_bookmark_name:''}}
			...
			<CardBody style={{padding:'10px 0 0 0'}}>
				<Collapse in={store.sr.bkgopen}>
					...
					<BookingBody type="M" mergeBkgProps={(data)=>store.setSr(data)} sameFlag ={false} confirm={confirm} {...props}/>		 		
				</Collapse> 
				{viewBookmark}
			</CardBody>
		</Card>	  

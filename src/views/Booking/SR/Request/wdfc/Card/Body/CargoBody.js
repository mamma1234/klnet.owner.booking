...
function CargoBody(props) {
	...
	const onChangeMark =(value)=> {
		...
		axios.post("/shipper/getUserMarkBookmark",{user_no:store.sr.user_no?store.sr.user_no:'',seq:value},{}).then(res => {
			props.mergeCargoProps({...cargoData,mark_yn:'Y', ...res.data[0]}); console.log("mark:");
		});
	}
	...
	const onChangeGoods =(value)=> {
		...
		axios.post("/shipper/getUserGoodsBookmark",{user_no:store.sr.user_no?store.sr.user_no:'',seq:value},{}).then(res => {
			props.mergeCargoProps({...cargoData,goods_yn:'Y', ...res.data[0]})
		});
	}
	
	const onChangeStoreGoods =(value)=> {
		axios.post("/shipper/getUserGoodsBookmark",{user_no:store.sr.user_no?store.sr.user_no:'',seq:value},{}).then(res => {
			store.setSr({...res.data[0], goods_desc:res.data[0].goods_desc?fnc_transGoods(store.sr.trans_service_code,res.data[0].goods_desc):null});
		});
	}
					...
					<GridContainer>
						...
						<GridItem className={classes.grid} lg={7} md={9} sm={9} xs={12}>
							...
							inputProps={{
								...
								// onBlur: (e)=>e.target.value?'B'===type?props.mergeCargoProps(cargoData):store.setSr(cargoData):null,
								onBlur: (e)=>'B'===type?props.mergeCargoProps(cargoData):e.target.value && (e.target.value !== store.sr.cargo_hs_code)?store.setSr({cargo_hs_code:e.target.value}):null,
								style:{height:'30px'}
							}}/>
						</GridItem>
					</GridContainer>
								...
								<GridItem className={classes.grid} lg={4} md={9} sm={9} xs={12}>
									<CustomInput
										...
										inputProps={{
											...
											//onBlur: (e)=>e.target.value?'B'===type?props.mergeCargoProps(cargoData):store.setSr(cargoData):null,
											onBlur: (e)=>'B'===type?props.mergeCargoProps(cargoData):e.target.value && (e.target.value !== store.sr.cargo_pack_qty)?store.setSr({cargo_pack_qty:e.target.value}):null,
											style:{height:'30px'}
										}}/>
								</GridItem>
							</GridContainer>
						...
						<GridItem className={classes.grid} lg={9} md={9} sm={9} xs={12}>
							<CustomInput
								...
								inputProps={{
									...
									//onBlur: (e)=>e.target.value?'B'===type?props.mergeCargoProps(cargoData):store.setSr(cargoData):null,
									onBlur: (e)=>'B'===type?props.mergeCargoProps(cargoData):e.target.value && (e.target.value !== store.sr.cargo_total_weight)?store.setSr({cargo_total_weight:e.target.value}):null,
									style:{height:'30px'}
								}}/>
						</GridItem>
					</GridContainer>
						...
						<GridItem  className={classes.grid} lg={9} md={9} sm={9} xs={12}>
							<CustomInput
								...
								inputProps={{
									...
									//onBlur: (e)=>e.target.value?'B'===type?props.mergeCargoProps(cargoData):store.setSr(cargoData):null,
									onBlur: (e)=>'B'===type?props.mergeCargoProps(cargoData):e.target.value && (e.target.value !== store.sr.cargo_total_volume)?store.setSr({cargo_total_volume:e.target.value}):null,
									style:{height:'30px'}
								}}/>
						</GridItem>
					</GridContainer>
			...
			<GridContainer>
				...
				<GridItem className={classes.grid} lg={7} md={7} sm={7} xs={12}>
					...
					<CustomOutInput
						...
						<InputLabel style={{ color: "#AAAAAA" }}>line:{cargoData.goods_desc?cargoData.goods_desc.split("\n").length:0} {' / '}

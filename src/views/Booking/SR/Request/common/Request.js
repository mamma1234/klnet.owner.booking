...
function SR(props) { 
	...
	const getServiceTerm =()=>{
		...
		axios.post("/shipper/selectLineCodeServiceType",{ params:{line_code:'WDFC'} }).then(res => 
			store.setSr_term_code(res.data));//setServiceTerm(res.data));
	}

	//컴펌 부킹 조회
	const getUserBookingList = () => {
		if(params) {
			axios.post("/shipper/getUserBookingInfo",{user_no:props.userData?props.userData.user_no:'',bkg_no:''},{})								
			.then(res => store.setSr_Confirm(res.data));
		}
	}

	// 전체 북마크 조회
	const onBookmark =()=>{
		axios.post("/shipper/getUserBookmark",{user_no:props.userData?props.userData.user_no:''}).then(res => {//setBookmark(res.data)
		...
		axios.post("/shipper/getUserSRDataList",{user_no :props.userData?props.userData.user_no:'',data:params,link:params?params.sr_no?'Y':'N':'N',list:'N'}).then(res => {
			if(res.data) {
				if(store.sr.sameflag) {
					...
					store.setSr({...res.data,noti_name1:'',noti_name2:'',noti_address1:'',noti_address2:'',noti_address3:'',noti_address4:'',noti_address5:''});
				} else {
	...
	const cntrTypeSize = ()=> {
		axios.post("/shipper/selectLineCodeCntrSztp",{params:{'line_code':'WDFC'}},{}).then(res => 
			store.setSr_size_type(res.data)//setCntrSztp(res.data);
		);
	}
	...
	const onHandleSrDocNew =()=>{
		...
		axios.post("/shipper/getUserSrDocInit",{user_no :props.userData?props.userData.user_no:'',sr_no:null,klnet_id:props.userData?props.userData.klnet_id:''}).then(res => { 
			srNo=res.data.sr_no;
			if(store.sr.sameflag) {
	...
	const onHandleSrDocSelf =()=>{
		...
		axios.post("/shipper/getUserSrDocInit",{user_no :props.userData?props.userData.user_no:'',sr_no:srNo,klnet_id:props.userData?props.userData.klnet_id:''}).then(res => {
			setConfirm(false);
			if(store.sr.sameflag) {
				//setstore.sr.({...res.data,'bkglist':[],noti_name1:'SAME AS CONSIGNEE',noti_name2:'',noti_address1:'',noti_address2:'',noti_address3:'',noti_address4:'',noti_address5:''});
				store.setSr({...res.data,'bkglist':[],noti_name1:'SAME AS CONSIGNEE',noti_name2:'',noti_address1:'',noti_address2:'',noti_address3:'',noti_address4:'',noti_address5:''});
	...
	const onHandleDupCheck = (sr)=>{
		srNo=sr;
		axios.post("/shipper/getUserNewSrDupCheck",{sr_no:srNo}).then(res => {
			if(!res.data) {
				setConfirm(true);
	...
	const onHandleSave =() => {
		...
		if(!store.sr.sr_no) {
			...
			setProgress(<Backdrop className={classes.backdrop} open={true}><CircularProgress color="primary" /></Backdrop>);
	...
	const allOpenCloseCard =(flag) => {
		if( !flag ) {
			document.getElementById("btnSend").scrollIntoView({behavior: "smooth",block: "start",inline: "nearest",});
		}
		setAllOpen(flag);
...
/* const onHandleSames=(flag)=>{
	...
	return(
			...
			<form>
				...
				<div style={{paddingLeft:'5%',paddingRight:'5%'}}>
					<GridItem xs={12} sm={12} style={{textAlign:'right'}}>
						...
						<Button id="btnReset" color="info" onClick={()=>onReset()}>RESET</Button>
					...
					<ContainerCard cntrProps={store.sr.cntrlist} reloadBookmark={()=>onBookmark()}
						{...props}/>
					{(store.sr.hbl_yn && store.sr.hbl_yn === 'N') &&
					<ExportCard exProps={store.sr.declarelist} reloadBookmark={()=>onBookmark()} {...props} />}
					...
					<ButtonGroup 
						style={{zIndex:'100',position:'fixed',right:'1%',top:'20%'}} 
		...
		<Button 
			id="btnTop" 
			className="mb-1 pt-1 pb-1" 
			style={{zIndex:'100',position:'fixed',right:'1%',top:'86%'}}
			color="rose" size="sm"
			//outline
			type="button" 
			onClick={(e) => {
			...
			}}><i className="fa fa-angle-double-up fa-3x" /><br/><span style={{position:'absolute',top:'64%',right:'15%',fontSize:'1px'}}>Top</span></Button>

...
function ScheduleBody(props) {
	...
	const handleSchChangePort = (prop,value) => { 
		setScheduleData({
			...scheduleData,
			[prop]:value, 
			[prop+"_name"]:(store.sr_start_port_code.findIndex(x=>x.value===value)>=0)?store.sr_start_port_code[store.sr_start_port_code.findIndex(x=>x.value===value)].port_name:(store.sr_end_port_code.findIndex(x=>x.value===value)>=0)?store.sr_end_port_code[store.sr_end_port_code.findIndex(x=>x.value===value)].port_name:''
		});
		store.setSr({
			...scheduleData,
			[prop]:value, 
								...
								<GridItem lg={5} md={5} sm={5} xs={12}>
									<CustomInput
										...
										inputProps={{
											...
											onBlur: (e)=>'B'===type?props.mergeSchProps(scheduleData):e.target.value && (e.target.value !== store.sr.sch_vessel_name)?store.setSr({sch_vessel_name:e.target.value}):null,style:{height:'30px'}
										}}
									/>
								</GridItem>/

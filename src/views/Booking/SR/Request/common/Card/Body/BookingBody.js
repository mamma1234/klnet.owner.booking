...
function BookingBody(props) {
	...
	const onChangeBookingProps = (value)=>{ 
		...
		if(value) {
			...
			if(value.line_code === 'WDFC') {
				var msg = "해당 부킹은 위동항운 부킹입니다. 위동항운 페이지로 이동합니다."
				...
				function  mergeConfirmBooking() {
					var list ={sr_no:store.sr.sr_no,sr_date:store.sr.sr_date,res_bkg_no:value.res_bkg_no,sc_no:value.sc_no,trans_service_code:value.trans_service_code,
						...
						sch_vessel_name:value.sch_vessel_name,sch_vessel_voyage:value.sch_vessel_voyage,sch_fdp:value.sch_fdp,line_code:value.line_code,
						sch_fdp_name:value.sch_fdp_name,sch_eta:value.sch_eta,sch_etd:value.sch_etd,shp_name1:value.shp_name1,
					...
					window.location.href="/svc/srWdfc?param="+JSON.stringify(list)
				}
				

				function  onCancleProps () {
					//props.mergeProps({...bookingData,res_bkg_no:value.res_bkg_no,'bk_link':'N','bkglist':[{value:value.value,res_bkg_no:value.res_bkg_no}]});
				...
				function  mergeConfirmBooking() {
					...
					//props.mergeProps({...bookingData,...list});
			...
			//props.mergeProps({...bookingData,res_bkg_no:'','bk_link':'N','bkglist':[{}]});
						...
						<GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
							<CustomInput
								...
								inputProps={{
									...
									onBlur: (e)=>e.target.value?props.mergeProps(bookingData):null,
									style:{height:'30px'}
								}}/>
						</GridItem>
					</GridContainer>
					...
					<GridContainer>
						...
						<GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
							<Autocomplete
								...
								onBlur ={(e)=>'B'===type?props.mergeProps(bookingData):e.target.value && (e.target.value !== store.sr.res_bkg_no)?store.setSr({res_bkg_no:e.target.value}):null}
								renderInput={
						...
						<GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
							<CustomInput
								...
								inputProps={{
									...
									onBlur: (e)=>'B'===type?props.mergeProps(bookingData):e.target.value && (e.target.value !== store.sr.sc_no)?store.setSr({sc_no:e.target.value}):null,
									style:{height:'30px'}
								}}/>
				    		...
				    		<GridContainer>
				              ...
				              <GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
					              <Autocomplete
					                  ...
					                  onChange={(e,option)=>'B'===type?props.mergeProps({...bookingData,bl_type:option?option.value:''}):store.setSr({bl_type:option?option.value:''})}
						              renderInput={
						                params =>(
						                  <TextField
				    		...
				    		<GridContainer>
				                ...
				                <GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
					                <Autocomplete
									  ...
									  onChange={(e,option)=>'B'===type?props.mergeProps({...bookingData,trans_service_code:option?option.value:''}):store.setSr({trans_service_code:option?option.value:'',
												   goods_desc:fnc_transGoods(option?option.value:'',store.sr.goods_desc?store.sr.goods_desc:'')})
									  }
				    		...
				    		<GridContainer>
				                ...
				                <GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
					                <Autocomplete
					                  ...
					                  onChange={(e,option)=> {
										  'B'===type?props.mergeProps({...bookingData,hbl_yn:option?option.value:''}):store.setSr({hbl_yn:option?option.value:''})
									  }}
						              renderInput={
						                params =>(
						                  <TextField
				    		...
				    		<GridContainer>
				                ...
				                <GridItem className={classes.grid} lg={8} md={7} sm={7} xs={12}>
					                <Autocomplete
					                  ...
					                  onChange={(e,option)=>'B'===type?props.mergeProps({...bookingData,line_payment_type:option?option.value:''}):store.setSr({line_payment_type:option?option.value:''})}
						              renderInput={
						                params =>(
						                  <TextField
						                    inputProps={{maxLength:1}}
...
export default React.memo(observer(BookingBody),areEqual);

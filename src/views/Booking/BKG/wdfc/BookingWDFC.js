...
// 이건 테스트 주석
import { Card, ButtonGroup, Paper, Checkbox, IconButton} from "@material-ui/core";
import {NoteAdd, Send, Save, Print, Delete, Cancel, OpenInNew } from "@material-ui/icons";
...
const BookingCommon = observer((props)=> {
    ...
    const [outPortCodeList, setOutPortCodeList] = useState([]);
    const [inPortCodeList, setInPortCodeList] = useState([]);
...
//   },[]);
  ...
  useEffect(() => {
    ...
    useEffect(()=>{
        if( bookingStore.isVessel) {
            let params = {
                line_code: 'WDFC',
                sch_vessel_name: bookingStore.isVessel
            }
            selectLineCodeVesselPickup(params);
    ...
    },[bookingStore.isVessel]);

    useEffect(()=>{
        if( bookingStore.isVesselPolPod.sch_vessel_name
            && bookingStore.isVesselPolPod.sch_pod
            && bookingStore.isVesselPolPod.sch_pol) {
            let params = {
                line_code: 'WDFC',
                sch_vessel_name: bookingStore.isVesselPolPod.sch_vessel_name,
                sch_pol: bookingStore.isVesselPolPod.sch_pod,
                sch_pod: bookingStore.isVesselPolPod.sch_pol,
            }
            selectLineCodeVesselPortCfs(params);
        }
    },[bookingStore.isVesselPolPod])
    ...
    const selectLinePort =(params)=> {
        axios.post("/shipper/selectLinePort",
        {
            params
        }).then(
            res =>{
                if( 'out' === params.key ) {
                    setOutPortCodeList(res.data);
                } else if ( 'in' === params.key ) {
                    setInPortCodeList(res.data);
                }
            }
        );
    }
...
//   }
  ...
  const fncAddContainer = (containerList) => {
...
console.log(parameter)
    ...
    const updateAllBooking = async(e)=> {
        ...
        if( !bookingStore.booking.bkg_no ) {
            props.alert(
                null // onConfirm: function
                , "부킹을 먼저 생성하세요." // title: string
                ,'warning' // default,info,success,warning,danger,error,input,custom,controlled
                ,true // show: boolean default:true
                ,true  // reverseButtons: boolean , default: false
                ,'lg' // btnSize: lg,sm,xs default:lg
        ...
        if( 'D' === bookingStore.booking.status_cud ) {
            props.alert(
                null // onConfirm: function
                , "해당 부킹은 삭제 되었습니다 저장 불가합니다." // title: string
                ,'warning' // default,info,success,warning,danger,error,input,custom,controlled
                ,true // show: boolean default:true
                ,true  // reverseButtons: boolean , default: false
                ,'lg' // btnSize: lg,sm,xs default:lg
    ...
    const sendBooking = (status)=> {
        ...
        if( status === 'SEND' ) {
            ...
            if( 'D' === bookingStore.booking.status_cud ) {
                props.alert(
                    null // onConfirm: function
                    , "해당 부킹은 삭제 되었습니다 전송이 불가합니다." // title: string
                    ,'warning' // default,info,success,warning,danger,error,input,custom,controlled
                    ,true // show: boolean default:true
                    ,true  // reverseButtons: boolean , default: false
                    ,'lg' // btnSize: lg,sm,xs default:lg
        ...
        props.alert(()=>{
            ...
            // Confirm 인 경우
            if( !validation.fncValidation() && status === 'SEND' ) {
                fncOpenCardInvalid();
                props.alert(
                    ()=>{} // onConfirm: function
        ...
        }, "CHECK_TERM_WDFC",'warning',true,true,'lg'
        ,message
        ,true // confirm 버튼
        ,true,
        null
    ...
    <React.Fragment>
        <div style={{paddingLeft:'5%',paddingRight:'5%'}}>
            <Card className={classes.booking_title}>
                <CardBody>
                    <GridItem xs={12} sm={12} style={{textAlign:'right'}}>
                        ...
                        <Button id="btnSend" color="info" onClick={(e)=>sendBooking('CANCEL')} endIcon={<Cancel/>}>CANCEL</Button>
            ...
            <ContainerCard
                ...
                isSchEtd={bookingStore.isSchEtd}
                isTerm={bookingStore.isTerm}
                ...
                lineCodeVesselCfs={lineCodeVesselCfs}
                fncOnBlurContainer={fncOnBlurContainer}
                fncAddContainer={fncAddContainer}

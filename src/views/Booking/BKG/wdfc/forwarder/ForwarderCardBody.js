...
const ForwarderCardBody = observer(({booking, ...props}) => {
  ...
  useEffect(()=>{
    ...
    if( 1 === props.forwarderCompanyList.length
      && !(forwarder.fwd_user_name || forwarder.fwd_user_email||forwarder.fwd_user_name || forwarder.fwd_user_tel
      || forwarder.fwd_code || forwarder.fwd_name1|| forwarder.fwd_name2|| forwarder.fwd_user_dept|| forwarder.fwd_user_fax)) {
      setForwarder({
        ...forwarder,
  ...
  useEffect(()=> {
    if( forwarder.fwd_business_number) {
      ...
      axios.post(
          ...
          res => {
              ...
              if( res.data.length>0 ) {
                ...
                //  else {
                //     setForwarder({...forwarder, ['fwd_business_number']:null, ['fwd_name1']:null, ['fwd_address1']:null, ['fwd_user_name']:null, ['fwd_user_email']:null});
                //     fncOnBlurParent({...forwarder, ['fwd_business_number']:null, ['fwd_name1']:null, ['fwd_address1']:null, ['fwd_user_name']:null, ['fwd_user_email']:null});
                //   }
                // fncOnBlurParent(forwarder);
              }
          }
      );
      ...
      <GridContainer>
        ...
        <GridItem xs={12} sm={9}>
          <CustomInput
            ...
            required={"Y" === bookmarkYn? false:("Y" === forwarder.user_forwarder_yn)?true:false}
            feedback="forwarder"
          />
        </GridItem>
      </GridContainer>

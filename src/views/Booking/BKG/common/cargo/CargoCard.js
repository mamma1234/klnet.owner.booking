...
const CargoCard = observer(({cargo, goodsRelationList, ...props}) => {
  ...
  // },[props.booking]);
  useEffect(()=>{
    if( "Y" === cargo.cargo.selected_yn) {
      selectBookingCargoBookmarkRelation( cargo.cargo );
    }
  },[cargo.cargo]);
  ...
  // Cargo Bookmark 조회
  const selectBookingCargoBookmarkRelation = (params) => {
    axios.post(
        ...
        line_code: 'WDFC',
        cargo:{
          cargo_bookmark_seq: params.cargo_bookmark_seq
        }
    }
        ,{}
    ).then(

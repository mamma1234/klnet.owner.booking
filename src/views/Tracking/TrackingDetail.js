...
import {Select,Table,Grid,TableHead,TableRow,TableBody,TableCell,TableFooter,Popover,Box,Collapse} from "@material-ui/core";
...
import CurrentList from "views/Tracking/Current/TrackingCurrent.js";
import CntrListTable from "views/Tracking/Terminal/TrackingCntrList.js";
//import Slider from "components/Slide/Slider.js";
import Button from "components/CustomButtons/Button.js";
// page
import TerminalImp from "views/Tracking/TrackingImpTerminal.js";
import TerminalExp from "views/Tracking/TrackingExpTerminal.js";
...
export default function TrackingDetail (props) {
	  ...
	  const {
		  tableData, tableHeaderColor, tableRownum,curpage
	  } = props;
...
/*	  function  BodyData() {
	  ...
	  const handleCurPage = (event) => {
		...
		const fncSelectedGridRowDataMidMain = ( prop ) => {
			console.log( prop );
			props.fncSelectedGridRowDataMid(prop);
		}

		
	  
	  return ( 
		        <div  className={classes.tableResponsive2} style={{marginTop:'0px'}}>
			    	...
			    	<Table stickyHeader className={classes.table} style={{borderTop:'2px solid #00b1b7', borderBottom:'2px solid #00b1b7'}}>
			          <TableHead  className={classes[tableHeaderColor + "TableHeader"]} style={{padding:'5px'}} id="scroll_top">
			            <TableRow >
			                  <TableCell style={{paddingTop:'8px',paddingBottom:'8px',borderBottomWidth:'3px',width:'15%',fontWeight:'bold',color:'#717172'}} >(B/K NO.) B/L NO.<br/>Remark</TableCell>
			        ...
			        <TableBody >
			           {
			              tableData.map((prop, key) => {
			                  return (
			                    <RowTable key={key} index={key + 1} data={prop}  color={tableHeaderColor} handleRowSelectedFunction={fncSelectedGridRowDataMidMain} />
			                  );
			                })
			           }
...
function RowTable(props: { data: ReturnType<typeof createData> }) {
	...
	// 세관정보 API 수출용
		...
		const handleRowSelectedFunction = (prop) => {
			// console.log("==================");
			// console.log(prop.data);
			// console.log("==================");
			// setSelectedRowData(prop.data);
			props.handleRowSelectedFunction(prop.data);
		}

	  return (
	    <React.Fragment>
		  <TableRow key={props.index} 
		  		style={{borderCollapse:'separate',borderSpacing:'2px 2px',paddingTop:'5px'}} 
				  onClick={(event)=>handleRowSelectedFunction(props)}
				  hover>
	        <TableCell className={classes.tableMainDtlCell} style={{paddingTop:'0',paddingBottom:'0',textAlignLast:'left',borderBottom:borderBottomStyleValue}}>{data.view_bl_bkg}{bookmarkIcon === 'Y'?<Tooltip title="BookMark"><StarIcon onClick={handleStarClick} style={{color:'#00acc1',verticalAlign:'bottom'}} /></Tooltip>:<Tooltip title="BookMark"><StarBorderIcon onClick={handleStarClick} style={{color:'#00acc1',verticalAlign:'bottom'}} /></Tooltip>}<br></br>
				  {
				  data.remark!==null?data.remark.indexOf(',')!==-1?
				  (
					<div>
				  		<span>{data.remark.substr(0,data.remark.indexOf(',',0))}</span>
						<Tooltip title={data.remark.split(',').map((element) => {return <div>{element}<br></br></div>})}>
						<SmsIcon/>
						</Tooltip>
					</div>
				  ):data.remark:""}
	        ...
	        <TableCell style={{paddingTop:'0',paddingBottom:'0',borderBottom:borderBottomStyleValue}}>
	        	<Tooltip title={data.line_nm}>
	            	<a target="_blank" href={data.line_url}>
	        			{data.image_yn=='Y'?<img src={require("assets/img/carrier/"+data.line_code+".gif")} alt={data.line_code} style={{backgroundColor:'silver'}}/>:data.line_nm?data.line_nm:data.line_code}
	        		</a>
	        	</Tooltip>
	        </TableCell>
	        <TableCell style={{paddingTop:'0',paddingBottom:'0',borderBottom:borderBottomStyleValue}}>
	        	{data.vsl_name?data.vsl_name:data.max_vsl_name}<br/>{data.voyage?"/"+data.voyage:""}
	        	...
	        	:data.max_vsl_name?<Tooltip title="vessel map infomation">
	        	<DirectionsBoatIcon size="20" style={{verticalAlign:'bottom',color:'#00acc1',width:'20px',height:'20px'}} 
	        	onClick={handleClickMapOpen} /></Tooltip>
	        	:null}</TableCell>
	          ...
	          <Collapse in={open} timeout="auto" unmountOnExit>
	            ...
	            {data.ie_type==="I"?
	    		          	<Grid container spacing={1} justify="space-between">
	    		          		...
	    		          		<div><CustomIcon style={{color:'#00acc1',verticalAlign:'bottom'}} />CUSTOMS</div>
	    						  <TablesUniPass
	    							  tableHeaderColor={color}

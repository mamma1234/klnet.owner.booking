...
export default function ScheduleList(props) {
  ...
  const onSubmit = () => {
	...
	if(props.userData) {
		if(tapNum===0) {
			numCnt=1;
			axios.post("/com/	",{cntrNo:cntrNo,num:numCnt})
			.then(setImportData([]))
			...
			numCnt=1;
			axios.post("/com/getExpFlowSample",{cntrNo:cntrNo,num:numCnt})
				.then(setExportData([]))
	...
	if(props.userData) {
		...
		if(tapNum===0) {
			if(numCnt !== importData[0].tot_page) {
				numCnt=numCnt+1; 
				axios.post("/com/getImpFlowSample",{cntrNo:cntrNo,num:numCnt})
...
function ImpTable(props) {
	  ...
	  const { tableData, //tableHeaderColor, 
		tableRownum } = props;

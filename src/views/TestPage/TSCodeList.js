						...
						onRowAdd: newData =>
							...
							new Promise(resolve => {
								setTimeout(() => {
									...
									axios.post("/sch/getTSCodeList",{data:newData}).then(res => {
										if ( res.data.length === 0) {
											if(props.userData){
												axios.post("/sch/insertTSCode",{newData:newData}).then(res => {
										...
										if(props.userData){
											axios.post("/sch/getTSCodeList",{data:newData}).then(res => {
												...
												if ( res.data.length === 0) {
													...
													axios.post("/sch/updateTSCode",{ oldData:oldData, newData:newData});
													setSelectData(prevState => {
							...
							new Promise(resolve => {
								setTimeout(() => {
									...
									setSelectData(prevState => {
										...
										// 삭제 처리.
										if(props.userData){
											axios.post("/sch/deleteTSCode",{ oldData:oldData})
										}else {
											props.openLogin();
										}
										setSearchCount(data.length);
